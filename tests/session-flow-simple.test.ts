...
import { workoutRepository } from "../src/lib/repositories";
...
async function testCreateSession() {
  ...
  assertEqual(
    session.exercises[0].sets.length,
    3,
    "Exercise should have 3 sets"
  );
  assertEqual(
    session.exercises[0].sets[0].status,
    "todo",
    "Initial set status should be todo"
  );
  assertEqual(
    session.exercises[0].sets[0].reps,
    10,
    "First set should have 10 reps"
  );
  assertEqual(
    session.exercises[0].sets[0].weight,
    135,
    "First set should have 135 weight"
  );
...
async function testCompleteSet() {
  ...
  const updatedSet = await workoutRepository.updateSet(setId, {
    reps: 12,
    weight: 145,
    status: "complete",
  });
...
async function testPersistence() {
  ...
  const session = await workoutRepository.getSessionWithDetails(sessionId);

  assertDefined(session, "Session should be retrieved");
  assertEqual(
    session.exercises.length,
    1,
    "Session should still have 1 exercise"
  );
  assertEqual(
    session.exercises[0].sets.length,
    3,
    "Exercise should still have 3 sets"
  );
  ...
  assertEqual(
    firstSet.status,
    "complete",
    "First set status should be complete"
  );
  assertEqual(firstSet.reps, 12, "First set should have updated reps (12)");
  assertEqual(
    firstSet.weight,
    145,
    "First set should have updated weight (145)"
  );

  assertEqual(
    session.exercises[0].sets[1].status,
    "todo",
    "Second set should still be todo"
  );
  assertEqual(
    session.exercises[0].sets[2].status,
    "todo",
    "Third set should still be todo"
  );
...
async function testCompleteAdditionalSets() {
  ...
  await workoutRepository.updateSet(secondSetId, {
    reps: 10,
    weight: 155,
    status: "complete",
  });
  ...
  await workoutRepository.updateSet(thirdSetId, {
    reps: 8,
    weight: 185,
    status: "complete",
  });
  ...
  const updatedSession =
    await workoutRepository.getSessionWithDetails(sessionId);
  assertDefined(updatedSession, "Updated session should be retrieved");

  assertEqual(
    updatedSession.exercises[0].sets[0].status,
    "complete",
    "First set should be complete"
  );
  assertEqual(
    updatedSession.exercises[0].sets[1].status,
    "complete",
    "Second set should be complete"
  );
  assertEqual(
    updatedSession.exercises[0].sets[2].status,
    "complete",
    "Third set should be complete"
  );
...
async function testLastValues() {
  ...
  const lastValues =
    await workoutRepository.getLastValuesForExercise("Bench Press");

  assertDefined(lastValues, "Last values should be returned");
  assert(
    lastValues.reps > 0,
    "Last values should have reps greater than 0"
  );
  assert(
    lastValues.weight > 0,
    "Last values should have weight greater than 0"
  );

  console.log(
    `    ✓ Last values retrieved: ${lastValues.reps} reps @ ${lastValues.weight} lbs`
  );
}
...
async function testAddExercise() {
  ...
  const newExerciseId = await workoutRepository.addExerciseToSession(
    sessionId,
    {
      name: "Overhead Press",
  ...
  assert(
    newExerciseId.length > 0,
    "New exercise ID should be returned"
  );
  ...
  assertEqual(
    session.exercises.length,
    2,
    "Session should now have 2 exercises"
  );
  assertEqual(
    session.exercises[1].name,
    "Overhead Press",
    "Second exercise should be Overhead Press"
  );
  assertEqual(
    session.exercises[1].sets.length,
    2,
    "Second exercise should have 2 sets"
  );
...
async function testCompleteSession() {
  ...
  const updatedSession = await workoutRepository.updateSessionStatus(
    sessionId,
    "completed"
  );
  ...
  assertEqual(
    session.status,
    "completed",
    "Session status should be persisted as completed"
  );
...
async function testListSessions() {
  ...
  const sessions = await workoutRepository.listRecentSessionsForUser(
    DEFAULT_USER_ID,
    10
  );
  ...
  assertEqual(
    newSession.exercises.length,
    2,
    "Session should have 2 exercises"
  );
...
async function testGetCompletedSessions() {
  ...
  const completedSessions =
    await workoutRepository.getRecentCompletedSessions(DEFAULT_USER_ID, 5);

  assertDefined(completedSessions, "Completed sessions should be returned");
  assert(
    completedSessions.length > 0,
    "Should have at least one completed session"
  );
...
async function testCreateAndDeleteSet() {
  ...
  const newSetId = await workoutRepository.createSet(firstExerciseId, {
    reps: 5,
    weight: 205,
  ...
  let updatedSession = await workoutRepository.getSessionWithDetails(sessionId);
  assertDefined(updatedSession, "Updated session should be retrieved");
  assertEqual(
    updatedSession.exercises[0].sets.length,
    initialSetCount + 1,
    "Set count should increase by 1"
  );

  await workoutRepository.deleteSet(newSetId);

  // Small delay for Convex propagation
  await new Promise((resolve) => setTimeout(resolve, 100));

  const finalSession = await workoutRepository.getSessionWithDetails(sessionId);
  assertDefined(finalSession, "Final session should be retrieved");
  assertEqual(
    finalSession.exercises[0].sets.length,
    initialSetCount,
    "Set count should return to original"
  );
